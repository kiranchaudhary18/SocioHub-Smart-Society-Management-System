...
interface Props {
  bills: UtilityBill[];
  onViewDetails: (bill: UtilityBill) => void;
}

export function UtilityBillsTable({ bills, onViewDetails }: Props) {
  if (bills.length === 0) {
    return (
      <div className="bg-white/60 backdrop-blur-xl border border-white/80 rounded-[32px] p-16 shadow-sm text-center flex flex-col items-center justify-center">
        ...
        <h3 className="text-xl font-bold text-slate-800 mb-2">No Utility Bills</h3>
        <p className="text-sm font-medium text-slate-500 max-w-md">There are no utility bills recorded for this month.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white/60 backdrop-blur-xl border border-white/80 rounded-[32px] p-6 shadow-[0_4px_20px_rgba(0,0,0,0.03)] overflow-hidden">
      <div className="flex items-center gap-3 mb-6 px-2">
        ...
        <h3 className="text-xl font-heading font-black text-slate-800 tracking-tight">Itemized Charges</h3>
      </div>
        ...
        <table className="w-full text-left border-collapse min-w-[900px]">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="px-4 py-3 text-xs font-bold text-slate-400 uppercase tracking-wider">Bill Number</th>
              <th className="px-4 py-3 text-xs font-bold text-slate-400 uppercase tracking-wider">Utility Type</th>
              <th className="px-4 py-3 text-xs font-bold text-slate-400 uppercase tracking-wider">Billing Month</th>
              <th className="px-4 py-3 text-xs font-bold text-slate-400 uppercase tracking-wider">Consumption</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {bills.map((bill) => (
              <tr key={bill.billNumber} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="px-4 py-4">
                  <span className="text-sm font-bold text-slate-800">{bill.utilityType}</span>
                </td>
                <td className="px-4 py-4">
                  <span className="text-sm font-medium text-slate-600">{bill.billingMonth}</span>
                </td>
                <td className="px-4 py-4">
                  <span className="text-sm font-bold text-slate-700">{bill.consumption}</span>
                </td>
                <td className="px-4 py-4">
                  <div className="flex flex-col">
                    <span className="text-sm font-black text-slate-800">₹{(bill.amount + bill.tax).toLocaleString()}</span>
                    <span className="text-xs font-medium text-slate-500">Includes taxes</span>
                  </div>
                </td>
                <td className="px-4 py-4">
                  <span className={cn(
