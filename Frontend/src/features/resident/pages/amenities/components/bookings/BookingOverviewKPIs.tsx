...
import type { BookingOverviewKPIs as KPIsType } from "../../../../services/amenity.service";

interface Props {
  kpis: KPIsType;
}

export function BookingOverviewKPIs({ kpis }: Props) {
  const data = [
    { label: "Upcoming", value: kpis.upcoming, icon: CalendarClock, color: "text-[#3DD9FF]", bg: "bg-[#3DD9FF]/10", desc: "Future reservations" },
    { label: "Completed", value: kpis.completed, icon: CheckCircle2, color: "text-[#72F1D1]", bg: "bg-[#72F1D1]/10", desc: "Past visits" },
    { label: "Cancelled", value: kpis.cancelled, icon: XCircle, color: "text-red-400", bg: "bg-red-400/10", desc: "Reverted bookings" },
    { label: "This Month", value: kpis.thisMonth, icon: CalendarDays, color: "text-[#8F7CFF]", bg: "bg-[#8F7CFF]/10", desc: "Total activity" },
  ];
