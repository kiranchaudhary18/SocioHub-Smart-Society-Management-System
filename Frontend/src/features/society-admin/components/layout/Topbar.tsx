    ...
    <header className="h-20 w-full shrink-0 flex items-center justify-between px-8 relative z-40 mt-4">
      ...
      <div className="flex-1 max-w-md hidden md:flex items-center">
        <nav className="flex items-center space-x-2 text-sm font-medium text-slate-500 bg-white/60 backdrop-blur-xl border border-white/80 px-4 py-2.5 rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
          <Link to="/admin" className="hover:text-[#72F1D1] transition-colors flex items-center gap-1.5">
          ...
          {paths.slice(1).map((segment, index) => {
            ...
            // Skip ID segments (basic UUID/number check for cleaner breadcrumbs)
            ...
            return (
              <div key={path} className="flex items-center space-x-2">
                ...
                {isLast ? (
                  ...
                  <Link to={path} className="hover:text-[#72F1D1] transition-colors">
      ...
      {/* Center: Search (Visible on mobile instead of breadcrumbs) */}
      <div className="flex-1 max-w-md md:hidden relative group">
        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
          <Search className="h-4 w-4 text-slate-400 group-focus-within:text-[#72F1D1] transition-colors" />
        </div>
        <input
          type="text"
          className="w-full h-11 bg-white/60 backdrop-blur-xl border border-white/80 text-slate-800 text-sm rounded-2xl pl-11 pr-4 shadow-[0_8px_30px_rgba(0,0,0,0.03)] focus:outline-none focus:ring-4 focus:ring-[#72F1D1]/10 focus:border-[#72F1D1]/40 transition-all placeholder:text-slate-400 font-medium"
          placeholder="Search..."
        />
      </div>
      ...
      <div className="flex items-center gap-4 sm:gap-6 ml-auto">
        ...
        <div className="hidden sm:flex items-center gap-2 px-3 py-2 rounded-2xl bg-white/60 backdrop-blur-xl border border-white/80 shadow-sm cursor-pointer hover:bg-white/80 transition-colors tooltip-trigger" title="Switch Society">
          <Building className="w-4 h-4 text-[#3DD9FF]" />
        ...
        <div className="relative group">
          <button className="flex items-center gap-3 p-1 pr-3 rounded-2xl bg-white/60 backdrop-blur-xl border border-white/80 shadow-sm hover:bg-white/90 transition-all cursor-pointer">
            <div className="w-9 h-9 rounded-xl overflow-hidden bg-gradient-to-br from-[#72F1D1] to-[#3DD9FF] p-0.5">
            ...
            <div className="hidden md:flex flex-col items-start">
              ...
              <span className="text-[10px] font-bold text-slate-500 leading-tight uppercase tracking-widest mt-0.5">Society Admin</span>
            </div>
          </button>
          ...
          <div className="absolute right-0 top-full mt-2 w-48 bg-white/80 backdrop-blur-2xl border border-white/80 shadow-[0_8px_30px_rgba(0,0,0,0.08)] rounded-2xl p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all translate-y-2 group-hover:translate-y-0 z-50">
            <Link to="/admin/profile" className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-slate-50 text-slate-600 hover:text-[#72F1D1] font-bold text-sm transition-colors">
